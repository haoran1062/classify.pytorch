...
def train_model(model, dataloaders, criterion, optimizer, num_epochs=25, epoch_start=0, save_base_path='./', save_step=500, logger=None, vis=None):
    since = time.time()
        ...
        for phase in dataloaders.keys():
            ...
            for it, temp in enumerate(dataloaders[phase]):
                ...
                if it % 10 == 0:
                    ...
                    vis.img('result', get_show_result_img(labels.to('cpu').numpy()[0], preds.to('cpu').numpy()[0]))
...
if __name__ == "__main__":
    ...
    optimizer_ft = optim.Adam(model_p.parameters(), lr=1e-3, eps=1e-8, betas=(0.9, 0.99), weight_decay=0.)
    ...
    model_p.train()
    # Train and evaluate
    train_model(model_p, dataloaders, criterion, optimizer_ft, num_epochs=config_map['epoch_number'], epoch_start=config_map['resume_epoch'], save_base_path=config_map['model_save_path'], logger=logger, vis=my_vis)

...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0, num_classes=20, with_wh=False):
        ...
        # Linear layer
        if with_wh:
            self.whclassifier = nn.Linear(num_classes + 2, num_classes)
        
        self.originclassifier = nn.Linear(num_features, num_classes)
    ...
    def forward(self, x, w=None, h=None):
        ...
        cls_pred = self.originclassifier(out)
        if w is not None and h is not None:
            w = w.view(features.shape[0], -1)
            h = h.view(features.shape[0], -1)
            # print(out.shape, w.shape, h.shape)
            # print(out.dtype, w.dtype)
            cls_wh_out = torch.cat((cls_pred, w, h), dim=-1)
            # print(out.shape)
            cls_wh_pred = self.whclassifier(cls_wh_out)
            return cls_pred, cls_wh_pred
